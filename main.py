...
async def auto_sender():
    ...
    targets = [t.strip() for t in Prompt.ask("Target (User/ID, pisah koma)").split(",")]
    ...
    def get_panel(last_msg, cd, count):
        t = Table.grid()
        t.add_row(f"[bold green]Status:[/bold green] Mengirim...")
        ...
        while total < limit:
            ...
            try:
                for target in targets:
                    ...
                    if len(targets) > 1: await asyncio.sleep(2)
                total += 1
